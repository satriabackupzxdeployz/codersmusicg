...
function CanvasExportCard({
  ...
  return (
    <div ref={forwardRef} className="canvas-export-root">
...
export default function App() {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      // Memutar musik tetap dilanjutkan di background; tidak ada aksi pause otomatis di sini.
      // Browser modern + Media Session API menjaga audio tetap berjalan walau tab tidak aktif.
      if (document.visibilityState === "visible" && "mediaSession" in navigator) {
        navigator.mediaSession.playbackState = isPlaying ? "playing" : "paused";
      }
    };
  ...
  const openCollection = async (item: PipedCollection) => {
    ...
    try {
      ...
      setCollectionDetail({
        meta: { ...item, title: detail.title || item.title, artwork: detail.artwork || item.artwork, count: detail.count || item.count },
